...
class ZeroSpeedBehaviorTests(unittest.TestCase):
    ...
    def test_availability_probe_batch_continues_to_normal_queue_selection(self):
        client = FakeQbtClient([
            ...
            },
            {
                ...
                "availability": 2.0,
                "has_metadata": True,
                "tags": "availability-verified-20990101T000000Z",
            },
        ])
        env = {
            "QBT_AVAILABILITY_ADMISSION_ENABLED": "true",
        ...
        actions = [
            ...
            for line in stdout.getvalue().splitlines()
            if line.startswith("{")
            and json.loads(line).get("event") == "qbt_guard_decision"
        ]
        self.assertIn("availability_admitted", actions)
        self.assertIn("try_candidate", actions)
    ...
    def test_apply_single_download_parks_zero_speed_torrent_after_wait_without_bytes(self):
        ...
        self.assertEqual("zero", try_event["winner_torrent"]["hash"])
        self.assertIsNone(try_event["runner_up_torrent"])
        self.assertEqual("zero", try_event["current_active_torrent"]["hash"])
        self.assertIn("score_breakdown", try_event["selected_torrent"])
        self.assertIn("queue", try_event["selected_torrent"]["score_breakdown"])
        self.assertIn("health", try_event["selected_torrent"]["score_breakdown"])
        self.assertIn("observed_download_speed_bytes_per_sec", try_event["selected_torrent"]["score_breakdown"])
        self.assertEqual("zero", park_event["selected_torrent"]["hash"])
        self.assertEqual("zero", park_event["winner_torrent"]["hash"])
    ...
    def test_category_workers_honor_total_active_download_cap(self):
        torrents = [
            {
                "hash": f"{category}-{index}",
                "name": f"{category.title()}.{index}",
                "category": category,
                "state": "stoppedDL",
                "dlspeed": 0,
                "amount_left": 1000,
        ...
        self.assertEqual(4, len(client.started[-1]))
        self.assertIn((4, None), client.queue_limits)
        self.assertEqual(
            4,
            try_event["candidate_counts"][
                "normal_category_max_total_active_downloads"
            ],
        )
        self.assertEqual(
            4,
            try_event["candidate_counts"]["normal_category_total_worker_limit"],
        )
        self.assertEqual(
            4,
            try_event["candidate_counts"]["normal_category_workers"],
        )
    ...
    def test_apply_single_download_preempts_productive_for_better_balanced_candidate(self):
        ...
        self.assertEqual("challenger", preempt_event["winner_torrent"]["hash"])
        self.assertEqual("current", preempt_event["runner_up_torrent"]["hash"])
        self.assertEqual("current", preempt_event["current_active_torrent"]["hash"])
        self.assertIn("score_breakdown", preempt_event["winner_torrent"])
        self.assertIn("queue", preempt_event["winner_torrent"]["score_breakdown"])
        self.assertIn("health", preempt_event["winner_torrent"]["score_breakdown"])
        self.assertIn("candidate score", preempt_event["reason"])
        self.assertIn("score", preempt_event["rejected_torrents"][0]["torrent"])
        self.assertIn("score_breakdown", preempt_event["rejected_torrents"][0]["torrent"])
    ...
    def test_selection_lease_keeps_recent_peer_worker_over_higher_score_candidate(self):
        ...
        self.assertIn("selection lease active", keep_event["reason"])
        self.assertEqual(1, keep_event["candidate_counts"]["selection_lease_active"])
        self.assertEqual("current", keep_event["winner_torrent"]["hash"])
        self.assertEqual("challenger", keep_event["runner_up_torrent"]["hash"])
        self.assertEqual("current", keep_event["current_active_torrent"]["hash"])
        self.assertIn("score_breakdown", keep_event["winner_torrent"])
        self.assertNotIn(["current"], client.stopped)
